...
struct SearchBar: View {
    ...
    var body: some View { HStack(spacing: 10) { Image(systemName: "magnifyingglass").font(.title3); TextField(placeholder, text: $text).font(.subheadline).textInputAutocapitalization(.never).autocorrectionDisabled().submitLabel(.search).focused(focus ?? $editing).onSubmit { if let focus { focus.wrappedValue = false } else { editing = false } }.accessibilityIdentifier("universal-search"); if !text.isEmpty { Button { text = "" } label: { Image(systemName: "xmark.circle.fill").foregroundStyle(BarTheme.muted).frame(width: 44, height: 44) }.accessibilityLabel("Clear search") } }.padding(.leading, 16).padding(.trailing, 8).frame(minHeight: 54).background(BarTheme.card, in: RoundedRectangle(cornerRadius: 16)).overlay(RoundedRectangle(cornerRadius: 16).stroke(BarTheme.stone, lineWidth: 0.6)).toolbar { ToolbarItemGroup(placement: .keyboard) { Spacer(); Button("Done") { if let focus { focus.wrappedValue = false } else { editing = false } } } } }
}
