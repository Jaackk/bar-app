                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        VStack(alignment: .leading, spacing: 20) {
                            ...
                            if let reference = drink.sourceReference { Label(reference, systemImage: "doc.text").font(.caption).foregroundStyle(BarTheme.muted) }
                            if let source = drink.sourceURL, let url = URL(string: source) { Link(drink.isHouseClassic ? "Source spreadsheet" : "Official Hove drinks menu", destination: url).font(.subheadline) }
